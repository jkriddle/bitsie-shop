...
namespace Bitsie.Shop.Api
{
    public class CoinbaseConnector
    {
        ...
        public CoinbaseConnector(string apiKey, string apiSecret)
        {
            API_KEY = apiKey;
        ...
        public string CreateOrder(string name, decimal price, string callbackUrl = "")
        {
            var order = new JObject();
            var button = new JObject();
            button.Add("name", name);
            button.Add("type", "buy_now");
            button.Add("price_string", price);
            button.Add("price_currency_iso", "USD");
            if (!String.IsNullOrEmpty(callbackUrl)) button.Add("callback_url", callbackUrl);
            order["button"] = button;
            
            return JsonRequest(URL_BASE + "orders", order, POST);
        }

        public string GetOrder(string orderNumber)
        {
            var o = GetOrders();
            return JsonRequest(URL_BASE + "orders/" + orderNumber, GET);
        }

        public string GetOrders()
        {
            return JsonRequest(URL_BASE + "orders", GET);
        }
        ...
        private string JsonRequest(string url, JObject data, string method)
        {
            // take care of any spaces in params
            url = Uri.EscapeUriString(url);
